...
Item {
    ...
    function checkMenuIteration(idTxt) {
        ...
        for(var i = 0; i < automationChildren; i++) {
            result = trackAudioModel.get(ind).automationList.get(i).displayText === idTxt
            if(result) break
        }
        return result
    }
    ...
    Menu {
        ...
        Menu {
            ...
            Action {
                ...
                onTriggered: {
                    displayMenu = text
                    if(!isRootMenu) trackAudioModel.get(ind).automationList.setProperty(index, 'displayText', displayMenu)
                }
            }
            Action {
                text: qsTr("Pan")
