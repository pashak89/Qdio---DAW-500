...
*/
{
    ...
    // otherwise, apply fall time
        ...
        float fReleaseCoef = 20.0f * fTimePassed / 1.7f;
...
*/
{
    ...
    // otherwise, test whether to apply fall time
        ...
        // (negative values) OR the peak meter's hold time of 1.7
        // seconds has not yet been exceeded, so retain old peak level
        // mark
        if (fLastChanged < 1.7f) {
            fOutput = fPeakOld;
        }
        // peak meter's hold time (1.7 seconds) has been reached or
        // exceeded
        else {
            // fall time: 26 dB in 3 seconds (linear)
            float fReleaseCoef = 20.0f * fTimePassed / 1.7f;
