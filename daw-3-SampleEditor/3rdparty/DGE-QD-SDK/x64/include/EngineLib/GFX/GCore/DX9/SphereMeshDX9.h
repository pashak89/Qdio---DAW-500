...
namespace DGE
{
	...
	namespace GFX
	{
		...
		namespace DirectX9
		{
			...
			class DGE_GFXAPI SphereMeshDX9 : /*public BoundingVolume,*/ public BaseMesh
			{
			public:
				SphereMeshDX9(float Radius, float Slice, float Stack, bool IsDynamic = false,
							  const DChar* MaterialPath = nullptr,
							  const DChar* MaterialName = nullptr);
				SphereMeshDX9(float Radius, float Slice, float Stack, Vector3D v, float mass = 100,
							  bool IsDynamic = false, const DChar* MaterialPath = nullptr,
							  const DChar* MaterialName = nullptr);
				virtual ~SphereMeshDX9();
			};
