...
namespace DGE
{
	namespace GFX
	{
		namespace Primitives
		{
			...
			class DGE_GFXAPI Segments : public DGE::GFX::Primitives::Geometry
			{
				D_OBJECT(Segments)
			public:
				Segments();
				Segments(const std::vector<Core::Math::Vector3D>& points);
			...
			protected:
				D_PROPERTY(std::vector<Core::Math::Vector3D>, points, Points,
						   std::vector<Core::Math::Vector3D>{});
			};
