...
const QTransform QNanoPainter::currentTransform() const
{
    ...
    QTransform t(static_cast<float>(xform[0]),
            static_cast<float>(xform[1]),
            static_cast<float>(xform[2]),
            static_cast<float>(xform[3]),
            static_cast<float>(xform[4]),
            static_cast<float>(xform[5]));
    delete [] xform;
    return t;
}
...
const QRectF QNanoPainter::textBoundingBox(const QString &text, float x, float y, float maxWidth)
{
    ...
    return QRectF(static_cast<float>(bounds[0]),
            static_cast<float>(bounds[1]),
            static_cast<float>(bounds[2]-bounds[0]),
            static_cast<float>(bounds[3]-bounds[1]));
}
