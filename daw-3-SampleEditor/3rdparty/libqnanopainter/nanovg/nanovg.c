...
{
	float invdet, det = (float)t[0] * (float)t[3] - (float)t[2] * (float)t[1];
	if (det > -1e-6 && det < 1e-6) {
		nvgTransformIdentity(inv);
		return 0;
	}
	invdet = 1.0 / det;
	inv[0] = (float)((float)t[3] * invdet);
	inv[2] = (float)((float)-t[2] * invdet);
	inv[4] = (float)(((float)t[2] * (float)t[5] - (float)t[3] * (float)t[4]) * invdet);
	inv[1] = (float)((float)-t[1] * invdet);
	inv[3] = (float)((float)t[0] * invdet);
	inv[5] = (float)(((float)t[1] * (float)t[4] - (float)t[0] * (float)t[5]) * invdet);
	return 1;
}
	...
	for (i = 0; i < ctx->cache->npaths; i++) {
		...
		if (path->nfill) {
			...
			for (j = 0; j < path->nfill; j++)
				printf("%f\t%f\n", (float)path->fill[j].x, (float)path->fill[j].y);
		}
		if (path->nstroke) {
			printf("   - stroke: %d\n", path->nstroke);
			for (j = 0; j < path->nstroke; j++)
				printf("%f\t%f\n", (float)path->stroke[j].x, (float)path->stroke[j].y);
		}
	}
}
